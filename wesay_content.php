...
<body>
    ...
    <div class="ui grid" ng-app="myApp" ng-controller="myCtrl">

        <div class="eight wide column">
            <div class="ui piled segment" ng-repeat="item in content" ng-if="item.type == <?=$_GET['id']?>">>
                <a class="ui blue ribbon label">{{ item.title }}</a>
                <h4 ng-bind-html="item.descriptions | to_trusted">
                </h4>
            </div>
        </div>

        <div class="eight wide column" id="result_data"> 
            <h1 id="text" ng-if="item.type == <?=$_GET['id']?>" ng-repeat="item in data">{{ item.item }}</h1>
        </div>
